...
export default class MetaPageManager extends LightningElement {
    ...
    enrichPages(raw) {
        return (raw || []).map(p => ({
            ...p,
            initials:    (p.Page_Name__c || 'P').substring(0, 2).toUpperCase(),
            statusClass: p.Status__c === 'Connected' ? 'badge badge-success' : 'badge badge-error',
            Forms_Count__c: p.Forms_Count__c || 0
        }));
    }

    get hasPages()       { return this.pages && this.pages.length > 0; }
    get totalPages()     { return this.pages.length; }
    get connectedPages() { return this.pages.filter(p => p.Status__c === 'Connected').length; }
    get totalForms()     { return this.pages.reduce((s, p) => s + (p.Forms_Count__c || 0), 0); }
}